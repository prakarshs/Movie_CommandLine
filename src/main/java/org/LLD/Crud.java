...
import org.LLD.Constants.Enums.Filters.MovieFilter;
            ...
            switch (input[0]) {
                ...
                case Commands.SHOW_ALL: {
                    if (input[1].equals(String.valueOf(ShowFilterType.actor))) {
                        ...
                        if (input[2].equals(String.valueOf(MovieFilter.none))) {
                            movieCrudService.listAllMovies();
                            break;
                        }

                        if (input[2].equals(String.valueOf(MovieFilter.by_genre))) {
                            movieCrudService.listMovieByGenre(MovieGenre.valueOf(input[3]));
                            break;
                        }
                        Integer id = Integer.parseInt(input[3]);
                        if (input[2].equals(String.valueOf(MovieFilter.by_actor))) {

                            movieCrudService.listMovieByActor(id);

                        } else if (input[2].equals(String.valueOf(MovieFilter.by_director))) {

                            movieCrudService.listMovieByDirector(id);

                        } else {
                            System.out.println("!----! Error In Movie Filter !-----!");
                        }
                    } else {
                        System.out.println("!----! Error In Show Type. Choose Between Actor,Director,Movie !-----!");

                    }
                }
                break;
                default: {
                    System.out.println("THERE HAS BEEN AN ERROR IN COMMANDS");
                }
            }
        }
    }
